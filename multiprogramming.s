...
# KERNEL DATA SEGMENT
#
# Data used by the exception and interrupt handler, i.e, the kernel.
###############################################################################
...
__job_0_context: .space 20 
__job_1_context: .space 20
...
main:
	# Initialize the simple multiprogramming system. 
...
# USER LEVEL JOBS
#
# Jobs are only allowed to use the following registers: $v0, $a0, $s0
###############################################################################
...
job_increment:
	...
	# NOTE: getjid is a non-blocking system call. 
...
BREAKPOINT_0:
	...
	# Execute the built in version of the getc system call
...
job_getc_infinite_loop:
	...
	# NOTE: getc is a blocking system call. 
...
TODO_1:	# TODO 1: Get address of the instruction (teqi) causing the exception. 
	# and store it in $t0. 
...
TODO_2:	# TODO 2: To resume at the instruction following the teqi instruction by 
	# adding 4 to EPC and store the result in $t1. 
	
	# TIP: Use the addi (Add Immediate) instruction. 
	
	
TODO_3:	# TODO 3: The value of EPC + 4 must now be saved in the context of the caller. 
	...
	# Job id of job to resume while the calling job waits. 
   	...
   	# Set EPC to PC value stored in job context.
	...
	# Change status of the resumed jov to running. 
...
__kbd_interrupt:
	...
	# Ignore the interrupt if job waiting for the getc system call to complete.
...
__getc_system_call_pending:
	...
	li $k1, 0xffff0004  # Address of RECEVIVER DATA
	
TODO_4:	# TODO 4: Before resuming the waiting job, put the ASCII value of the pressed
	# key in register $v0. 
